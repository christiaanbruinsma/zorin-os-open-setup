# Gradia

**Category:** Utilities → Screenshot Enhancement<br>
**Website:** [https://gradia.alexandervanhee.be/](https://gradia.alexandervanhee.be/)

Use this app to make your screenshots ready for sharing. Gradia helps you beautify, annotate, and share screenshots seamlessly, allowing you to quickly modify screenshots of application windows, add backgrounds, annotations, and prepare them for sharing with friends, colleagues, or a professional audience.

## Key Features
- Beautify and enhance screenshots
- Add backgrounds to screenshots
- Add annotations to screenshots
- Modify screenshots of application windows
- Prepare screenshots for professional sharing
- Seamless sharing workflow

## How to Get It
Gradia is available in the Zorin OS Software app and can also be downloaded from the official website.

[Get Gradia](https://gradia.alexandervanhee.be/)

## Privacy & Notes
- Open-source application (GPLv3)
- Developed by Alexander Vanhee
- Part of GNOME Circle
- Available on Flathub and Snap Store

---
*Created: 17-08-2026*
*Last updated: 17-08-2026*

---
*Information only. Not affiliated with or endorsed by app developers. Verify current features and information, as apps may change over time.*